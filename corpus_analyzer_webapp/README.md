# SEA Media Corpus Analyzer (Web)

Веб-приложение для загрузки медиакорпуса (ZIP папкой или отдельные `.txt/.md`), запуска строгого анализатора и скачивания всех результатов в одном ZIP.

## Возможности
- Загрузка корпуса:
  - `ZIP` архив (внутри любые подпапки с `.txt/.md`)
  - или по одному/нескольким файлам
